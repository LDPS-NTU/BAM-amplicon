...
void	alignment_SinglePoint(uint8_t *stream, alignmentHeader *AlignmentHeader, posCoverage *PosCoverage, uint32_t position , uint32_t ref_length);

int	BamSinglePoint(FILE *file_bam_i, FILE *file_bai_i, char *chromosome, uint32_t position, toolsFlags *ToolsFlags){
	...
	memset(&PosCoverage,0,sizeof(posCoverage));
...
//	printf("%lu\t%lu\t%lu\n", offset_bgzf, offset_decomp, counter);
	...
	while ( (len_data = BGFZBlock(file_bam_i)) > 0 ){
		...
		while ( top - address >= (AlignmentHeader.block_size + 4) ){
			...
			if ((AlignmentHeader.FLAG&1024)==0 || ToolsFlags->flag_dup==0 ){
				alignment_SinglePoint(address, &AlignmentHeader, &PosCoverage, position, chr_length[ref_ID]);
			}
			//Core_END
	...
	printf("%u\t",PosCoverage.A);
	printf("%u\t",PosCoverage.C);
	printf("%u\t",PosCoverage.G);
	printf("%u\t",PosCoverage.T);
	printf("%u\t",PosCoverage.N);
	printf("%u\n",PosCoverage.DEL);
	...
	for (i = 0;i < AlignmentHeader->n_cigar_op;i++){
		...
		if (CIGAR(op) == 'M'){
			...
			if (index <= position && position < index+op_len){
				...
				if ( residue == 'A'){		PosCoverage->A	+= 1; 
				}else if ( residue == 'C'){	PosCoverage->C  += 1;
				}else if ( residue == 'G'){	PosCoverage->G  += 1;
				}else if ( residue == 'T'){	PosCoverage->T  += 1;
				}else if ( residue == 'N'){	PosCoverage->N  += 1;
				}
			//	printf("%s\t%c\t%d\n",read_name, residue, op_len);
				break;
			}
			index += op_len;
			length += op_len;
