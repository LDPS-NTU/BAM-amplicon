...
void	alignment_GapTxt(uint8_t *stream, alignmentHeader *AlignmentHeader, FILE *file_deletion_o, uint32_t ref_length);
...
//					Coverage = calloc(chr_length[AlignmentHeader.refID],sizeof(uint32_t));
					strcat_triple(filename, "", chr_name[AlignmentHeader.refID], "_gap.txt", name_len);
			...
			//Core_START
			alignment_GapTxt(address, &AlignmentHeader, file_cov_o, chr_length[ref_ID]);
			//Core_END
